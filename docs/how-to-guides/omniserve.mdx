...
## Advanced: Middlewares and Resilience Patterns

OmniServe supports complex production architectures out-of-the-box using the powerful `omnicoreagent.omni_agent.omni_serve` modules.

### Customizing Middleware

You can attach additional FastAPI middlewares directly to OmniServe before starting it. This is useful for injecting custom request IDs, custom logging, or additional authentication protocols.

```python
from fastapi.middleware.cors import CORSMiddleware
from omnicoreagent import OmniServe

server = OmniServe(agent, config=config)

# Add your custom FastAPI middleware
...
### Circuit Breakers and HTTP Retry Backoff

When a local tool communicates with brittle third-party APIs or a flaky database, import our custom `RetryConfig` and `CircuitBreaker`. This will prevent your agent from indefinitely hanging when external systems are down.
...
breaker = CircuitBreaker("payment_api", failure_threshold=3, timeout=60)

@tools.register_tool("charge_card")
async def charge_card(amount: float):
    # If the payment API fails 3 times, the breaker trips for 60 seconds
    # Subsequent calls immediately return failure to the agent instead of blocking
    async with breaker:
        return await risky_payment_call(amount)
```

<Tip>
OmniServe is perfect for deploying agents as highly available microservices. Using builtin Circuit Breakers prevents sweeping production outages.
</Tip>
